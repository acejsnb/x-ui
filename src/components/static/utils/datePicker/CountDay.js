...
class CountDay {
    ...
    dayCode(M, F) {
        ...
        for (let i=1; i<=num; i++) {
            // n表示正常显示
            if (yy === this.Y && Number(this.M) === M && F==='n' && Number(this.D) === i) selected='selected';
            else selected='';
            arr.push({flag: F, selected, year: yy, month: mm, day:i<10?`0${i}`:`${i}`});
        }
    */
        let arr=[];
        for (let i=1; i<=num; i++) {
            // n表示正常显示
            arr.push({flag: F, selected: '', multiple: '', year: yy, month: mm, day:i<10?`0${i}`:`${i}`});
        }
        return arr;
    }
    ...
    getDaysArray() {
        const m=Number(this.M),
            da=[...this.countPrevWeek(m), ...this.dayCode(m, 'n')],
